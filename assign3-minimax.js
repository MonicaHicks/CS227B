...
function bestmove(state) {
  ...
  for (var i = 0; i < actions.length; i++) {
    ...
    var result = minimaxDepth(newState, 3);
...
//===============================================================
function minimaxDepth(state, role, depth) {
  if (findterminalp(state, library)) {
    return findreward(role, state, library) * 1;
  }
  if (depth <= 0) {
    return pessimistic(state, library);
  }
  var active = findcontrol(state, library);
  if (active === role) {
    return maximize(state, role, depth - 1);
  }
  return minimize(state, role, depth - 1);
}

function maximize(state, role, depth) {
  var actions = findlegals(state, library);
  if (actions.length === 0) {
    return 0;
  }
  ...
  for (var i = 0; i < actions.length; i++) {
    var newstate = simulate(actions[i], state, library);
    var newscore = minimaxDepth(newstate, role, depth);
    if (newscore === 100) {
      return 100;
    }
    if (newscore > score) {
...
function minimize(state, role, depth) {
  var actions = findlegals(state, library);
  if (actions.length === 0) {
    return 0;
  }
...
function pessimistic(state, role, library) {
  if (findterminalp(state, library)) {
...
function intermediate(state, role, library) {
  return findreward(role, state, library);
}
